...
import { Mic, X, AlertCircle, BookOpen, Activity } from 'lucide-react';
...
export function AIReader() {
  ...
  const [subStatus, setSubStatus] = useState('Mikrofonni yoqing...');
  const [hint, setHint] = useState("Matn shu yerda ko'rinadi...");
  ...
  // --- VIBRATSIYA VA VIZUAL SIGNAL ---
  const triggerFeedback = () => {
    // 1. Vibratsiya (Android va qo'llab-quvvatlaydigan qurilmalarda)
    // 200ms lik 2 ta qisqa tebranish
    if (typeof navigator !== 'undefined' && navigator.vibrate) {
      navigator.vibrate([100, 50, 100]);
    }

    // 2. Vizual "Silkinish" effekti (Barcha qurilmalar uchun)
    setIsShaking(true);
  ...
  const connectWebSocket = () => {
    ...
    // ... onerror, onclose (avvalgidek qoladi)
  };
  ...
  // ... startRecording, stopRecording (avvalgidek PCM 16k mantiqi)
  ...
  return (
    <div className={`min-h-screen bg-[#0f172a] flex flex-col items-center relative overflow-hidden font-sans transition-colors duration-300 ${isShaking ? 'bg-red-950/30' : ''}`}>
      ...
      {/* 1. Orqa Fon (Yengilroq animatsiya) */}
      ...
      {/* 3. Asosiy Kontent - Scroll bo'lishi mumkin */}
      <div className={`flex-1 w-full max-w-lg flex flex-col items-center justify-center p-6 mt-10 transition-transform duration-100 ${isShaking ? 'translate-x-[-5px] translate-y-[5px]' : ''} ${isShaking ? 'animate-[shake_0.5s_ease-in-out]' : ''}`}>
        ...
        {/* Mikrofon Tugmasi (Optimizatsiya qilingan) */}
        <div className="relative mb-12 group">
            {/* Aktiv holatdagi halqalar */}
            {isRecording && (
                <>
      ...
      {/* Shake Animation Keyframes (Global CSS ga qo'shish kerak yoki shu yerda ishlaydi) */}
      <style >{`
        @keyframes shake {
