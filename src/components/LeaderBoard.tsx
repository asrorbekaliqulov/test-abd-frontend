        ...
        try {
            ...
            setLeaderboardData(
                (response.data.results || []).sort(
                    (a: any, b: any) => b.created_tests - a.created_tests
                )
            );
        } catch (err: any) {
            console.error("Leaderboard error:", err);
            setError(
                err.response?.data?.detail || err.message || "Leaderboard yuklashda xatolik yuz berdi"
            );
        } finally {
            setLoading(false);
        }
    ...
    const toggleFollow = async (user: LeaderboardUser) => {
        try {
            ...
            setLeaderboardData((prev) =>
                prev.map((u) =>
                    u.id === user.id ? { ...u, is_following: res.is_following } : u
                )
            );
        } catch (err) {
                ...
                <div className="overflow-x-auto">
                    <table className="min-w-[700px] w-full text-left border-collapse">
                        ...
                        {leaderboardData.map((user, index) => (
                            ...
                            >
                                <td className="py-3 px-4 text-yellow-300 font-bold">{getMedal(index + 1)}</td>
